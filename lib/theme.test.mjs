...
import { THEME_COLORS, THEME_INIT_SCRIPT, THEME_OPTIONS, isDarkTheme, isThemePreference } from "./theme.ts";
  ...
  for (const systemDark of [false, true]) {
    for (const stored of [...THEME_OPTIONS.map(({ id }) => id), null, "", "unknown", new Error("Blocked")]) {
      ...
      runInNewContext(THEME_INIT_SCRIPT, {
        ...
        document: { documentElement: root, querySelector: () => ({ setAttribute: (_, value) => { themeColor = value; } }) },
      });
      ...
      assert.equal(themeColor, THEME_COLORS[expected]);
    }
  }
});
